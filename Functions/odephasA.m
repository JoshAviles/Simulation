function status = odephasA(t,y,flag,varargin)
...
if nargin < 3 || isempty(flag) % odephas2(t,y) [v5 syntax] or odephas2(t,y,'')
                ...
                if (oldi == 1) || ...
                    ...
                    if doDrawnow                         
                        if TARGET_HGCLASS  % start new segment           
                            if ~ishold
                                hold on 
                                plot(ud.y(ploti:newi,1),ud.y(ploti:newi,2),'-o','MarkerSize',3);        
                                hold off
                            else
                                plot(ud.y(ploti:newi,1),ud.y(ploti:newi,2),'-o','MarkerSize',3);        
                            end
    ...
    switch(flag)
      case 'init'                           % odephas2(tspan,y0,'init')
        ...
        if ~ishold
            ud.lines = plot(y(1),y(2),'-o','MarkerSize',3);
            hold on
            ud.line = plot(y(1),y(2),'-o','MarkerSize',3,'Color',co(2,:),EraseMode{:});
            hold off
        else
            ud.lines = plot(y(1),y(2),'-o','MarkerSize',3,EraseMode{:});
            ud.line = plot(y(1),y(2),'-o','MarkerSize',3,'Color',co(2,:),EraseMode{:});
        end
